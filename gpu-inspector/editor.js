...
module.exports = class GpuInspectorPlugin extends EditorPlugin {
  onEnable(editor) {
    this.addCommand({
      ...
      ? React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              "div",
              null,
              `CPU Build: ${snapshot.frame.cpuBuildMs.toFixed(2)} ms`
            ),
            React.createElement(
              "div",
              null,
              `CPU Submit: ${snapshot.frame.cpuSubmitMs.toFixed(2)} ms`
            ),
            React.createElement(
              "div",
              null,
              `GPU Duration: ${
                snapshot.frame.gpuMs !== undefined
                  ? snapshot.frame.gpuMs.toFixed(2) + " ms"
                  : "Unavailable"
              }`
            )
          ),
          React.createElement(
            ...
            null,
            `VRAM: ${(
              snapshot.resources.estimatedVramBytes /
              (1024 * 1024)
            ).toFixed(2)} MB | Textures: ${
              snapshot.resources.totalTextures
            } | Buffers: ${snapshot.resources.totalBuffers}`
          )
        )
      : React.createElement(
          "div",
          { style: { color: "#888" } },
          "Collecting GPU diagnostics..."
        )
  );
}
